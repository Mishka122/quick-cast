function getDeviceType({ mobileMax = 480, tabletMax = 992 } = {}) {
...
class Slider {
  ...
  init() {
    ...
    if (sliderPaginationParent) {
      if (totalSlides <= 1) {
        gsapWithCSS.set(sliderPaginationParent, {
          opacity: 0,
        });
      } else {
        for (let b = 1; b < totalSlides; b++) {
          ...
          if (!sliderCustom) {
            gsapWithCSS.set(sliderContentElements[b], {
              opacity: 0.4,
            });
          }
        }
    ...
    const unifiedAnimation = (step, direction) => {
      ...
      if (!sliderCustom) {
        gsapWithCSS.to(sliderContentParent, {
          duration: sliderDuration,
          ease: "power1.out",
          x: sliderWatcher * distance,
        });

        gsapWithCSS.to(currentSlide, {
          overwrite: true,
          ease: "power1.inOut",
          duration: 0.6,
          opacity: 1,
      ...
      accTop.addEventListener("click", () => {
        if (prevAccIndex === index) {
          ...
          if (prevAccIndex !== null) {
            ...
            const prevAccCloseTl = gsapWithCSS.timeline({
              overwrite: true,
              paused: true,
              defaults: { duration: 0.82, ease: "power2.out" },
            });
